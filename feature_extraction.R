...
set_up_poly2 <- function(specifications,    # list with named element: degree
	...
	return(list(
		...
		nrow = nrow(exponents),  # number of features
		x_default <- numeric(nrow(exponents)))  
		)
}


# get_x_poly extracts the features from a state-action combination fiven a specification from 'set_up_poly'
...
# get_x_poly_separate extracts the features from a state-action combination fiven a specification from 'set_up_poly_separate'
...
# available_prices <- seq(from = 1, to = 2.4, by = 0.2)
...
set_up_poly_tilings2 <- function(specifications,  # list with named elements regarding number of tiles and tilings
										  min_price,       # minimum price
										  max_price,       # maximum price
										  vars             # number of variables to enter the feature vector
) {
